...
| Section | Key Fields |
|---------|-----------|
| **Task Management** | Provider (linear/file/github), Status, Team ID / Repository, Project Number, Fallback |
...
| 2 | GitHub CLI | Run `gh auth status` + `gh repo view`. Both pass → github available |
| 3 | MCP Ref | Call `ref_search_documentation(query="test")`. Success → active |
| 4 | MCP Context7 | Call `resolve-library-id(libraryName="react")`. Success → active |
| 5 | File Editing tools | Per mcp_tool_preferences.md detection sequence |
| 6 | Agents | Run `codex --version` / `gemini --version`. Exit 0 → available |
| 7 | Git worktree | Run `git worktree list`. Success → available |
