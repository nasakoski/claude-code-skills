...
4. **Index codebase graph (if available):** IF `hex-graph` MCP server is available:
   ...
   - **Graph-assisted pattern discovery:** `find_symbols(query=GoF_suffix, kind="class")` -- find Factory*, Builder*, Strategy* etc. by structure, not just name. `find_implementations(symbol)` -- discover all implementations of abstract/interface classes.
