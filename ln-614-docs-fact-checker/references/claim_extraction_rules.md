...
grep -oP '`[a-zA-Z_./-]+\.[a-z]{1,5}`' "$doc"

# Markdown link targets
grep -oP '\]\(([^)]+\.md)\)' "$doc"

# Paths in prose (src/, app/, lib/, config/, tests/, docs/)
grep -oP '(?:src|app|lib|config|tests|docs)/[a-zA-Z0-9_/.-]+' "$doc"
```
...
```bash
# "N entities" pattern
grep -oP '(\d+)\s+(modules?|formats?|endpoints?|services?|tables?|parsers?|files?|workers?|routes?|columns?|containers?|domains?|categories?|layers?)' "$doc"

# "N+" pattern (lower bounds)
grep -oP '(\d+)\+?\s+(types?|checks?|rules?|tests?)' "$doc"
```
...
```bash
# HTTP method + path
grep -oP '(GET|POST|PUT|DELETE|PATCH)\s+/[\w/{}\-:.]+' "$doc"

# Table rows with endpoints
grep -oP '\|\s*(GET|POST|PUT|DELETE|PATCH)\s*\|\s*`?(/[\w/{}\-:.]+)`?' "$doc"
```
...
grep -oP '[A-Z][A-Z_]{2,}' "$doc" | sort -u

# process.env.X or os.environ references
grep -oP '(?:process\.env\.|os\.environ\[)[A-Z_]+' "$doc"

# Key=Value patterns
grep -oP '[A-Z_]{3,}\s*=' "$doc"
```
...
grep -oP '`(npm run \w+|python [\w.-]+|docker[\w -]+|make \w+|pip install[\w .-]+|cargo \w+)`' "$doc"
...
grep -oP '`[A-Z][a-zA-Z0-9]+`' "$doc"

# snake_case function references in backticks
grep -oP '`[a-z][a-z_0-9]+\(\)`' "$doc"

# Class/function in prose context
grep -oP '(?:class|function|method|service|handler)\s+`?([A-Z][a-zA-Z0-9]+)`?' "$doc"
```
...
# Port mappings
grep -oP '(\d{4,5}):(\d{4,5})' "$doc"

# Service names in docker context
grep -oP 'services?:\s*(\w+)' "$doc"
```
